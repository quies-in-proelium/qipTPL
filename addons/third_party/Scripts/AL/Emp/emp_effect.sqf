...

{
	...
	[
		[_x],
		QPATHTOF(Scripts\AL\Emp\sparky.sqf)
	] remoteExec ["execVM"];
	sleep _delay_check;
...
//{_x disableTIEquipment true; _x disableNVGEquipment true; [[_x],QPATHTOF(Scripts\AL\Emp\sparky.sqf)] remoteExec ["execVM"];sleep _delay_check} forEach static_turrets_emp;
