...
params ["_obj_nuke", "_rang_emp"];
private ["_poz_g", "_list_man", "_list_car", "_static", "_list_lit", "_special_helmet", "_special_launchers"];
...
sleep 30;
{
	...
	[[_x],QPATHTOF(Scripts\AL\Nuke\tgt_sfx.sqf)] remoteExec ["execVM"];
	sleep 0.1
} forEach _list_car;

{
	_x setDamage 0.9;
	[[_x],QPATHTOF(Scripts\AL\Nuke\sparky.sqf)] remoteExec ["execVM"];
	sleep 0.1
} forEach _list_lit;
{
	_x setDamage 1;
