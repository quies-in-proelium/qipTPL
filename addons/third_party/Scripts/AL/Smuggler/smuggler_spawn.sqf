...
params ["_spawn_obj_class","_object_anom_core"];
private ["_gigi","_spawn_obj","_spawn_obj_classname","_grp_side","_grp","_bounce_obj_temp","_tipat","_telep_in","_anim","_run_poz","_impact"];
...
while {!isNull _object_anom_core} do {
	...
	if (_spawn_obj_classname isKindOf "MAN") then {
		...
		if ((_spawn_obj distance _object_anom_core < 10) and (local _spawn_obj)) then {
			deleteVehicle _spawn_obj
		};
	};
};
