...
params ["_shower"];
private ["_action", "_target", "_remainingWater", "_diff", "_waterRemaining"];

if (local _shower) then {
    _shower setVariable ["cbrn_water", cbrn_maxWaterTime, true];
};
...
_action = ["cbrn_turn_on", "Turn on","",{
    ...
    _remainingWater = _target getVariable ["cbrn_water", cbrn_maxWaterTime];
    ...
    !(_target getVariable ["cbrn_on", false]) && {(_target getVariable ["cbrn_water", cbrn_maxWaterTime]) > 0}
...
_action = ["cbrn_turn_off", "Turn off","",{
    ...
    _diff = cba_missionTime - (_target getVariable ["cbrn_starTime", cba_missionTime]);
    _waterRemaining = ((_target getVariable ["cbrn_water", cbrn_maxWaterTime]) - _diff) max 0;
