...
# Uploads qipTPL relases to workshop
...
#GLOBALS
release_dir = "P:\\z\\qipTPL\\release"
project = "@qipTPL"
publisherTool_path = find_bi_tools()
changelog_path = os.path.join(release_dir,"changelog.txt")
qipTPL_release_dir = os.path.join(release_dir, project)
qipTPL_optionals_dir = os.path.join(qipTPL_release_dir, "optionals")
...
def main(argv):
    if not os.path.exists(qipTPL_release_dir):
        raise Exception("qipTPL_release_dir not found","qipTPL not built or in wrong path")
    if not os.path.exists(qipTPL_optionals_dir):
        raise Exception("qipTPL_optionals_dir not found","qipTPL not built or in wrong path")
    ...
    publishFolder(os.path.join(qipTPL_optionals_dir,"@qipTPL_compat_rhs_usf3"), "773125288", changelog_path)

    #RHS Compat Russians - http://steamcommunity.com/sharedfiles/filedetails/?id=773131200
    publishFolder(os.path.join(qipTPL_optionals_dir,"@qipTPL_compat_rhs_afrf3"), "773131200", changelog_path)

    #RHS Compat GREF - http://steamcommunity.com/sharedfiles/filedetails/?id=884966711
    publishFolder(os.path.join(qipTPL_optionals_dir,"@qipTPL_compat_rhs_gref3"), "884966711", changelog_path)
